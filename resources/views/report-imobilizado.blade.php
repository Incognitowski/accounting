  ...
  <div class="flex bg-grey-lighter w-screen h-screen">
    ...
    <div class="flex flex-1 items-start justify-center font-sans">
      <div class="md:mx-10 sm:mx-12 bg-white container items-center shadow-md rounded px-8 pt-6 pb-8 mb-4 mt-10 flex flex-col my-2">
        <h1 class='text-grey-dark w-full text-center border-b pb-3'>Relatório Geral de Imobilizados</h1>

        @if((count($custos)==0) and (count($receitas)==0) and (0>$depreciacao))
        ...
        <div class='w-full p-5 rounded flex bg-grey-lightest'>
          ...
          <div class='flex flex-col w-full border pb-5'>
            ...
            @if($depreciacao>0)
            <div class='account flex justify-between'>
              <p class='account-name md:px-5 lg:px-10 md:text-lg lg:text-xl font-medium text-red-dark'>Depreciação de Imobilizados</p>
              <p class='account-value md:px-5 lg:px-10 md:text-lg lg:text-xl font-medium text-red-dark'>R$ {{ number_format((float)$depreciacao, 2, ',', '.') }}</p>
            </div>
            @endif
          </div>
          <!--  -->
        ...
        <h2 class='text-grey-dark w-full text-center border-b pb-3'>Referente à {{ $data_inicial }} até {{ $data_final }}</h2>
        @endif
